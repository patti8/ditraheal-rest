...
-- membuang struktur untuk procedure layanan.CetakJobListLab
DROP PROCEDURE IF EXISTS `CetakJobListLab`;
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `CetakJobListLab`(
...
BEGIN
	 SET @sqlText = CONCAT('SELECT 
		...
		ref.DESKRIPSI CARA_BAYAR, lab.ALASAN DIAGNOSA, master.getNamaLengkapPegawai(dok.NIP) DOKTER
	...
	JOIN pendaftaran.kunjungan kjg ON kjg.REF = lab.NOMOR
	...
	IF PJENIS = 2 THEN	
		SET @sqlText = CONCAT('SELECT 
			...
			rasal.DESKRIPSI UNIT_SEBELUMNYA,
			IF(DATE_FORMAT(p.TANGGAL,"%d-%m-%Y")=DATE_FORMAT(kjg.MASUK,"%d-%m-%Y"),"Baru","Lama") STATUSPENGUNJUNG,
			DAFTAR_TINDAKAN TINDAKAN, lab.ALASAN DIAGNOSA, master.getNamaLengkapPegawai(dok.NIP) DOKTER,
