...
BEGIN
   ...
   END;
  	...
  	BEGIN    
	    ...
	    EXIT_EXEC: LOOP
	      FETCH CR_EXEC_DATA INTO VIDTT, VRUANG, VJUMLAH_RUANGAN, VJUMLAH, VTERPAKAI, VPREPARE, VPREPARE_PLAN, VCOVID;
	      ...
	      IF EXISTS(SELECT 1 FROM kemkes.data_tempat_tidur WHERE id_tt = VIDTT AND ruang = VRUANG) THEN
	        UPDATE kemkes.data_tempat_tidur 
	           SET jumlah_ruang = VJUMLAH_RUANGAN, jumlah = VJUMLAH, terpakai = VTERPAKAI, prepare = VPREPARE, prepare_plan = VPREPARE_PLAN, covid = VCOVID, kirim = 1
	          WHERE id_tt = VIDTT AND ruang = VRUANG;
	      ELSE
	        REPLACE INTO kemkes.data_tempat_tidur(id_tt, ruang, jumlah_ruang, jumlah, terpakai, prepare, prepare_plan, covid)
	            VALUES (VIDTT, VRUANG, VJUMLAH_RUANGAN, VJUMLAH, VTERPAKAI, VPREPARE, VPREPARE_PLAN, VCOVID);
	      END IF;
	    END LOOP;    
	    CLOSE CR_EXEC_DATA;
	END;    
