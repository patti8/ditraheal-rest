...
class ReservasiResource extends Resource
{
    ...
    {
        $data->OLEH = $this->user; 
        $tgl = System::getSysDate($this->service->getTable()->getAdapter(), false, true);
        if($this->service->tempatTidurTerpesan(
            $tgl
            , $data->RUANG_KAMAR_TIDUR)
        ) return new ApiProblem(406, 'Tempat tidur tersebut telah terpesan. Silahkan pilih tempat tidur yang lain.');
