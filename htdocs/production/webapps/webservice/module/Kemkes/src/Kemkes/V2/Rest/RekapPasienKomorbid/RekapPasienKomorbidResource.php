...
class RekapPasienKomorbidResource extends Resource
{
    protected $title = "Rekap Pasien Dirawat Komorbid";
        ...
        if(isset($params["loadFromWs"])) {
            if($params["loadFromWs"]) {                
                $result = $this->sendRequest([
                    "url" => $this->config["url"],
                    "action" => "LapV2/PasienDirawatKomorbid",
                    "method" => "GET"
                ]);
                
                if($result) {
                    foreach($result->RekapPasienDirawatKomorbid as $row) {
                        $row = (array) $row;
                        if(isset($row["status"])) {
    ...
    private function kirim(&$row) {
        ...
        if($result) {
            foreach($result->RekapPasienDirawatKomorbid as $_row) {
                $_row = (array) $_row;
