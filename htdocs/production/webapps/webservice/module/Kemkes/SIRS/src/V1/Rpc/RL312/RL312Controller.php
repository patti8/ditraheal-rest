...
class RL312Controller extends RPCResource
{
    protected $title = "RL 3.12";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitikduabelas?".http_build_query($params),
			]);
    ...
    public function delete($id)
    {
		...
		$result = $this->doSendRequest([
			"url" => $this->config['url'],
			"action" => "rltigatitikduabelas/".$_id,
			"method" => "DELETE"
		]);
