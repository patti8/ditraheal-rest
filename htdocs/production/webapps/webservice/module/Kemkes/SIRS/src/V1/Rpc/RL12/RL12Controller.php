...
class RL12Controller extends RPCResource
{
	protected $title = "RL 1.2";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rlsatutitikdua?".http_build_query($params),
			]);
			...
			try {
				...
				if($total > 0) $data = $this->service->load($params, ['*'], ['tahun DESC']);
			} catch(\Exception $e) {
				return [
					"status" => 412,
					"success" => false,
