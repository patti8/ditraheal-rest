...
class RL39Controller extends RPCResource
{
    protected $title = "RL 3.9";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiksembilan?".http_build_query($params),
			]);
    ...
    public function delete($id)
    {
		...
		$result = $this->doSendRequest([
			"url" => $this->config['url'],
			"action" => "rltigatitiksembilan/".$_id,
			"method" => "DELETE"
		]);
