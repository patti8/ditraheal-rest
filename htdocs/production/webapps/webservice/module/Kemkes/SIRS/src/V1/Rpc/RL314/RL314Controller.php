...
class RL314Controller extends RPCResource
{
    protected $title = "RL 3.14";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitikempatbelas?".http_build_query($params),
			]);
    ...
    public function kirimAction() {
	    ...
	    foreach($froms as &$row) {
	        ...
	        $result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitikempatbelas".$id,
				"method" => $method,
				"data" => $data
			]);
    ...
    public function delete($id)
    {
		...
		$result = $this->doSendRequest([
			"url" => $this->config['url'],
			"action" => "rltigatitikempatbelas/".$_id,
			"method" => "DELETE"
		]);
