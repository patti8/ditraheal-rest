...
class RL54Controller extends RPCResource
{
    protected $title = "RL 5.4";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rllimatitikempat?".http_build_query($params),
			]);
			...
			try {
				...
				if($total > 0) $data = $this->service->load($params, ['*'], ['tahun DESC', 'bulan', 'kode_icd_10', 'no_urut']);
			} catch(\Exception $e) {
				return [
					"status" => 412,
					"success" => false,
