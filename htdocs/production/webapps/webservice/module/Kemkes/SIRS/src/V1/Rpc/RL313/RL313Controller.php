...
class RL313Controller extends RPCResource
{
    protected $title = "RL 3.13";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiktigabelas?".http_build_query($params),
			]);
    ...
    public function kirimAction() {
	    ...
	    foreach($froms as &$row) {
	        ...
	        $result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiktigabelas".$id,
				"method" => $method,
				"data" => $data
			]);
    ...
    public function delete($id)
    {
		...
		$result = $this->doSendRequest([
			"url" => $this->config['url'],
			"action" => "rltigatitiktigabelas/".$_id,
			"method" => "DELETE"
		]);
