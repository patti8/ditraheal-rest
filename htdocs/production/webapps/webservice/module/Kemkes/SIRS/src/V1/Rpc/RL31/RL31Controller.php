...
class RL31Controller extends RPCResource
{
    protected $title = "RL 3.1";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiksatu?".http_build_query($params),
			]);
    ...
    public function kirimAction() {
	    ...
	    foreach($froms as &$row) {
			...
			$params["<_48_jam"] = $params["kurang_48_jam"];
			$params[">=_48_jam"] = $params["lebih_48_jam"];
			...
			unset($params["kurang_48_jam"]);
			unset($params["lebih_48_jam"]);
