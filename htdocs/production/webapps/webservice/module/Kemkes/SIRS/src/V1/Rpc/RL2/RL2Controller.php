...
class RL2Controller extends RPCResource
{
	protected $title = "RL 2";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rldua?".http_build_query($params),
			]);
			...
			try {
				...
				if($total > 0) $data = $this->service->load($params, ['*'], ['tahun DESC', 'no_kode']);
			} catch(\Exception $e) {
				return [
					"status" => 412,
					"success" => false,
