...
class RL51Controller extends RPCResource
{
    protected $title = "RL 5.1";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rllimatitiksatu?".http_build_query($params),
			]);
			...
			try {
				...
				if($total > 0) $data = $this->service->load($params, ['*'], ['tahun DESC', 'bulan', 'no']);
			} catch(\Exception $e) {
				return [
					"status" => 412,
					"success" => false,
