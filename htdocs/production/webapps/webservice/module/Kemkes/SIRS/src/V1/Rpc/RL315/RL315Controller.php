...
class RL315Controller extends RPCResource
{
    protected $title = "RL 3.15";
	...
	public function getList() {
		...
		if(!empty($queries["remote"])) {
			$result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiklimabelas?".http_build_query($params),
			]);
    ...
    public function kirimAction() {
	    ...
	    foreach($froms as &$row) {
	        ...
	        $result = $this->doSendRequest([
				"url" => $this->config['url'],
				"action" => "rltigatitiklimabelas".$id,
				"method" => $method,
				"data" => $data
			]);
    ...
    public function delete($id)
    {
		...
		$result = $this->doSendRequest([
			"url" => $this->config['url'],
			"action" => "rltigatitiklimabelas/".$_id,
			"method" => "DELETE"
		]);
