...
class AnamnesisResource extends Resource
{
    ...
    public function create($data)
    {
        $result = array(
			"status" => 422,
			"success" => false,
			"detail" => "Gagal menyimpan anamnesis"
		);
		...
		if($success) {
			...
			$result["detail"] = "Berhasil menyimpan anamnesis";
		}
    ...
    public function fetch($id)
    {
		...
		$result =  array(
			...
			"detail" => count($data) > 0 ? "Anamnesis ditemukan" : "Anamnesis tidak ditemukan"
		);
    ...
    public function fetchAll($params = array())
    {
		...
		if (isset($params["NOT"])) {
			...
			foreach($nots as $key => $val) {
				$params[] = new \Laminas\Db\Sql\Predicate\Expression("(NOT anamnesis.".$key." = ".$val.")");
			}
			unset($params["NOT"]);
		}
		...
		return array(
			...
			"total" => $total,
			"data" => $data,
			"detail" => $total > 0 ? "Anamnesis ditemukan" : "Anamnesis tidak ditemukan"
		);
    }
    ...
    public function update($id, $data)
    {
        $result = array(
			...
			"success" => false,
			"data" => null,
			"detail" => "Gagal merubah data anamnesis"
		);
		...
		if($canUpdate) {
			...
			if($success) {
				...
				$result["success"] = true;
				$result["data"] = $success[0];
				$result["detail"] = "Berhasil merubah data anamnesis";
			}
		}
