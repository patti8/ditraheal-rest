...
class RiwayatPemberianObatResource extends Resource
{
    ...
    public function create($data)
    {
        $result = [
			"status" => 422,
			"success" => false,
			"detail" => "Gagal menyimpan Riwayat Pemberian Obat"
        ];
		...
		if($success) {
			...
			$result["detail"] = "Berhasil menyimpan Riwayat Pemberian Obat";
		}
    ...
    public function fetch($id)
    {
		...
		$result = [
			...
			"detail" => count($data) > 0 ? "Riwayat Pemberian Obat ditemukan" : "Riwayat Pemberian Obat tidak ditemukan"
        ];
    ...
    {        
        ...
        if (isset($params["NOT"])) {
			...
			foreach($nots as $key => $val) {                
                $params[] = new \Laminas\Db\Sql\Predicate\Expression("(NOT riwayat_pemberian_obat.".$key." = ".$val.")");
		...
		return [
			...
			"total" => $total,
			"data" => $data,
			"detail" => $total > 0 ? "Riwayat Pemberian Obat ditemukan" : "Riwayat Pemberian Obat tidak ditemukan"
        ];
    }
    ...
    public function update($id, $data)
    {
        $result = [
			...
			"success" => false,
			"data" => null,
			"detail" => "Gagal merubah data Riwayat Pemberian Obat"
        ];
		...
		if($canUpdate) {
			...
			if($success) {
				...
				$result["success"] = true;
				$result["data"] = $success[0];
				$result["detail"] = "Berhasil merubah data Riwayat Pemberian Obat";
			}
		}
