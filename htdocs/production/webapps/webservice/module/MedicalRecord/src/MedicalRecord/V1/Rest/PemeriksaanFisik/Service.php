...
class Service extends DBService {
	...
	public function __construct($includeReferences = true, $references = array()) {
		$this->table = DatabaseService::get("SIMpel")->get(new TableIdentifier("pemeriksaan_fisik", "medicalrecord"));
		$this->entity = new PemeriksaanFisikEntity();
	...
	protected function query($columns, $params, $isCount = false, $orders = array()) {		
		...
		return $this->table->select(function(Select $select) use ($isCount, $params, $columns, $orders) {
			...
			if(!System::isNull($params, 'STATUS')) {
				...
				$params['pemeriksaan_fisik.STATUS'] = $status;
				unset($params['STATUS']);
			}
			...
			if(!System::isNull($params, 'HISTORY')) {
				...
				$select->join(array('k'=>new TableIdentifier('kunjungan', 'pendaftaran')), 'k.NOMOR = pemeriksaan_fisik.KUNJUNGAN', array());
