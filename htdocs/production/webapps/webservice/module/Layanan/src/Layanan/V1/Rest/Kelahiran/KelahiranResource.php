...
class KelahiranResource extends Resource
{
    ...
    public function create($data)
    {
        $result = array(
			"status" => 422,
			"success" => false,
			"detail" => "Gagal menyimpan data kelahiran"
		);
		...
		if($success) {
			...
			$result["detail"] = "Berhasil menyimpan data kelahiran";
		}
    ...
    public function fetch($id)
    {
		...
		$result =  array(
			...
			"detail" => count($data) > 0 ? "Data kelahiran ditemukan" : "Data kelahiran tidak ditemukan"
		);
    ...
    public function fetchAll($params = array())
    {
		...
		return array(
			...
			"total" => $total,
			"data" => $data,
			"detail" => $total > 0 ? "Data kelahiran ditemukan" : "Data kelahiran tidak ditemukan"
		);
    }
    ...
    public function update($id, $data)
    {
        $result = array(
			...
			"success" => false,
			"data" => null,
			"detail" => "Gagal merubah data kelahiran"
		);
		...
		if($canUpdate) {
			...
			if($success) {
				...
				$result["success"] = true;
				$result["data"] = $success[0];
				$result["detail"] = "Berhasil merubah data kelahiran";
			}
		}
