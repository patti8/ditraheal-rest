...
class PiutangPasienResource extends Resource
{
	public function __construct(){
		parent::__construct();
		$this->service = new PiutangPasienService();
	}
    /**
     * Create a resource
     *
    ...
    public function fetchAll($params = array())
    {
        if(isset($params['GET_TOTAL'])){
            if($params['GET_TOTAL'] == 'true'){
                ...
                $data = $this->service->execute("CALL pembayaran.getTotalPiutangPerorangan('".$params['NORM']."')");
                $total = count($data);    
            }
        }else{
