...
{    
	...
	private function cekUsersToken($xid,$xpass) {
		$db = DatabaseService::get("SIMpel");
		$adapter = $db->getAdapter();
		...
		$stmt = $adapter->query("
			SELECT *
			  FROM aplikasi.signature
			 WHERE X_ID = '".$xid."' AND md5(X_PASS) = md5('".$xpass."')");
		$results = $stmt->execute();
		return $results->current();
	}
	...
	public function generateTokenX($data, $xtimestamp) {
		$data = $this->cekUsersToken($data["X_ID"],$data["X_PASS"]);
		if($data) {
			$key = $data["X_ID"]."&".$xtimestamp;
			$result = array(
				"success" => true,
				"token" => base64_encode($key."&".md5($key).md5($data["X_PASS"]))
			);
			return $result;
		} else {
			$result = array(
