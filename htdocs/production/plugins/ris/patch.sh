...
if [ $# -lt 1 ]; then
	...
	echo "Example: bash patch.sh 1.0.0-20101600"
	exit 1
fi
...
if [ "$version" == "" ]; then	
	if [ -f /var/log/simpel-plugins-ris-latest.txt ]; then
		version=`cat /var/log/simpel-plugins-ris-latest.txt`
	fi	
fi

if [ "$version" == "test" ]; then
	if [ -f /var/log/simpel-plugins-ris-test.txt ]; then
		version=`cat /var/log/simpel-plugins-ris-test.txt`
	else
		version=""
	fi
fi
...
cat > /var/log/simpel-plugins-ris-test.txt <<-EOF
