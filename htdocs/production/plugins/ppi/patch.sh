...
if [ "$version" == "" ]; then	
	if [ -f /var/log/simpel-plugins-ppi-latest.txt ]; then
		version=`cat /var/log/simpel-plugins-ppi-latest.txt`
	fi	
fi

if [ "$version" == "test" ]; then
	if [ -f /var/log/simpel-plugins-ppi-test.txt ]; then
		version=`cat /var/log/simpel-plugins-ppi-test.txt`
	else
		version=""
	fi
fi
...
cat > /var/log/simpel-plugins-ppi-test.txt <<-EOF
